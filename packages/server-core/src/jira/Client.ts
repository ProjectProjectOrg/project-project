...
export type JiraTransportRequest = Readonly<{
  method: "GET" | "POST"
  url: string
...
export type JiraTransportResponse = Readonly<{
  status: number
  headers: Readonly<Record<string, string>>
  json: Effect.Effect<unknown, JiraError>
  stream: Stream.Stream<Uint8Array, JiraError>
}>

export type JiraTransportShape = Readonly<{
  execute: (
...
export class JiraTransport extends Context.Service<
  JiraTransport,
  JiraTransportShape
>()("@pp/server-core/jira/Client/JiraTransport") {}
...
type SnapshotMethod<F> = F extends (
  ...
  : never
export type JiraClientShape = Readonly<{
  snapshots: {
    [K in
      | "currentUser"
      | "project"
      ...
      | "watchers"
      | "votes"
      | "boardConfiguration"]: SnapshotMethod<JiraClientShape[K]>
  }
...
export class JiraClient extends Context.Service<JiraClient, JiraClientShape>()(
  "@pp/server-core/jira/Client/JiraClient"
) {}
