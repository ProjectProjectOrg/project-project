  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      rafIdRef.current = requestAnimationFrame(() => {
        ...
        const scrollOffset = axis === "x"
          ? container.scrollLeft
          : container.scrollTop
        const borderOffset = axis === "x"
          ? container.clientLeft
          : container.clientTop
        const containerEdge = axis === "x"
          ? containerRect.left
          : containerRect.top
        ...
        const layoutSize = axis === "x"
          ? container.offsetWidth
          : container.offsetHeight
        const visualSize = axis === "x"
          ? containerRect.width
        ...
        for (let index = 0; index < rects.length; index++) {
          ...
          const itemStart = containerEdge
            + (borderOffset + contentPos - scrollOffset) * scale
