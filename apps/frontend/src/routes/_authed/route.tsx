...
import type { ProjectIconImage, User } from "@pp/shared"
import {
  createFileRoute,
  Link,
  Navigate,
...
import { logout, me } from "@/features/auth/atoms/auth"
import {
  projectsFor,
  projectsRequest
    ...
    <div className="h-full transition-[padding] duration-300 ease-out motion-reduce:transition-none lg:p-3">
      <div className="grid h-full grid-cols-1 grid-rows-[3.5rem_1fr] overflow-hidden bg-background transition-[border-radius,box-shadow] duration-300 ease-out motion-reduce:transition-none lg:grid-cols-[14rem_1fr] lg:rounded-2xl lg:shadow-sm lg:ring-1 lg:ring-border/60">
        ...
        <main className="flex min-h-0 overflow-hidden p-2 pt-0">
          <div
            data-scroll-root
            className="min-w-0 flex-1 [scrollbar-gutter:stable] overflow-auto rounded-xl bg-muted/60"
          >
  ...
  return (
    ...
    >
      <Link
        ...
        params={{ orgSlug, slug }}
        className={cn(
          "flex min-w-0 flex-1 items-center gap-2.5 rounded-lg py-2 pr-1 pl-3 text-[13px] transition-colors",
          active
      ...
      >
        <span
          aria-hidden
          className={cn(
            "inline-flex size-4 shrink-0 items-center justify-center overflow-hidden rounded-[4px] text-[13px] leading-none transition-[filter,opacity] duration-150 corner-squircle",
      ...
      <Link
        ...
        title={settingsLabel}
        className={cn(
          "mr-1 grid size-7 shrink-0 place-items-center rounded-md text-muted-foreground transition-colors transition-transform duration-100 outline-none hover:bg-accent hover:text-foreground focus-visible:ring-1 focus-visible:ring-ring active:scale-[0.97]",
      ...
      <SheetTrigger
        render={
          <button
            ...
            className="grid size-9 shrink-0 place-items-center rounded-md text-muted-foreground transition-colors transition-transform duration-100 outline-none hover:bg-accent hover:text-foreground focus-visible:ring-1 focus-visible:ring-ring active:scale-[0.97] lg:hidden"
          >
