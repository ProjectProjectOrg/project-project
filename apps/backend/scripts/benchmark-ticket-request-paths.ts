...
import { Users, type UsersShape } from "@pp/server-core/users/Users"
import {
  encodeCursor,
  GroupDetail,
  GroupId,
...
const FakeUsers = Layer.succeed(Users, {
  ...
  findManyByIds: () => Effect.succeed([]),
  fullByIds: () => Effect.succeed([])
  ...
  Effect.gen(function* () {
    ...
    yield* measure("list-deep-cursor", () =>
      verify(
        ...
        (page) =>
          page.items.length > 0 &&
          page.items.every(
            ({ ticket }) => Number(ticket.id.slice(2)) > deepNumber
          ),
    ...
    yield* measure("list-filter-tag", () =>
      verify(
        ...
        (page) =>
          page.items.length > 0 &&
          page.items.every(({ ticket }) =>
            ticket.tags.includes(performanceTag)
          ),
    ...
    yield* measure("list-filter-group", () =>
      verify(
        ...
        (page) =>
          page.items.length > 0 &&
          page.items.every(
            ({ ticket }) => Number(ticket.id.slice(2)) % 4 === 0
          ),
...
async function main() {
  ...
  try {
    ...
    const host = await Effect.runPromise(
      Effect.gen(function* () {
        return yield* HostEnvironment
      }).pipe(Effect.provide(HostEnvironmentLive))
    )
    if (results.length === 0)
