      ...
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          yield* Effect.promise(async () => {
            ...
            const listedProjects = await first.callTool({
              name: "list_projects",
              arguments: { orgSlug: orgId }
            })
            expect(listedProjects).toMatchObject({
              content: [{ text: expect.stringContaining("mcp-published") }]
            })
            expect(listedProjects).not.toMatchObject({
              content: [{ text: expect.stringContaining("mcp-hidden") }]
            })
