...
Task 6 review amendment: internal `resumeScan`/scan `resume` returns schema-backed `Resumed | AwaitRetry { failureSequence } | Rejected`. Under the existing row lock it rejects stale execution/attempt, cleanup ownership, cancellation/terminal or nonrecoverable states, but returns `AwaitRetry` without mutation when the same execution has a strictly newer generation. This includes a newer retry already accepted into scanning or an allowed scanned configuration/ready/migrating replay state. The workflow iteratively awaits that exact generation and persists each generation-qualified resume decision; only `Resumed` advances the logical operation try. Exact-generation mutation predicates remain unchanged. No new coordinator/queue is introduced, and these undeployed internal results require no production-history migration.

`parentsSubtasks` represents true subtask relationships only: the child issue's normalized issue type must have `subtask: true`. Ordinary children of epics remain independently represented in epic membership, and source parent relationships remain in raw Jira provenance. Manifest validation rejects a non-subtask issue claimed as a subtask.

## Command protocol amendments before deployment (Task 7)

Run acceptance and public `DurableDeferred.succeed` execute in one shared PgClient transaction. The SQL projection and SQL workflow engine must use the same memoized PgClient instance. A failed submission rolls back both acceptance and the stored signal; observation starts after commit, is bounded to two seconds, and rereads the current detail on timeout. Task 7 proves SQL rollback and no premature delivery; Task 11 owns abrupt process interruption/restart evidence and production wiring.

The checkpoint carries a versioned current gate and an immutable accepted configuration snapshot with its accepted revision. Completing the scan installs StartImport for that scan even after scan retries. Import failures after acceptance install a fresh Retry gate; a corrected ready draft signals that gate rather than reusing the consumed StartImport. Operational retry preserves the accepted snapshot revision. Gate/snapshot writes preserve scan progress and failure receipts atomically.

After hidden reservation, configuration changes are limited to explicit additions of currently unresolved failed attachment IDs to the accepted skip list, with skip consent. Destination, mappings, restriction policy, and earlier skips remain unchanged; broader edits require reset/rescan. The materialization lookup is required and bound to the owned execution, attempt, and expected revision. Task 9 provides its real implementation.

`MigrationActivities` and `makeProjectionMigrationActivities` require a materialize callback. The workflow invokes it after StartImport and before returning the original migration identity. There is no production fallback; Task 7 tests provide explicit implementations, Task 9 provides real orchestration, and Task 11 performs production cutover.

Cleanup claim and release take an explicit internal mode, `reset_import` or `discard`. Reset may claim a hidden destination in a rescan-eligible state, including corrected ready/configuration and reconnect states, under the current revision and attempt fence. Discard retains failed/cancelled eligibility. Reset waits for verified removal and uses the returned post-cleanup fence/revision before starting the replacement scan; it must reject intervening commands. Delete-after-cleanup must not be broadened to ready/configuration states. Task 7 SQL tests cover these mode-specific guards and verified reset revision handoff.
