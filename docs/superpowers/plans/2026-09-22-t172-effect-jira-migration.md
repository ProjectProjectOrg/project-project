...
`Workflow.interrupt` does not itself drain active storage writes. Wouter chose an attempt-level durable checkpoint in the existing projection: set before scan/materialization storage writes, clear only after all writes have known terminal outcomes, and retain the migration recovery row if uncertainty remains. Task 9 owns the write boundary; Task 11 must verify absence before final row deletion. The earlier investigation is in `cleanup-write-boundary-notes.md`.
...
Task 11 remains in progress: real cleanup callbacks, a reset command adapter, isolated SQL workflow registration, and a configuration-pause SQL restart proof now exist. The server still uses the legacy worker; durable service cutover, worker removal, retention registration, and further restart cases are outstanding. Task 10 has a concurrent cancellation/publication integration assertion but still needs broader fault injection. Task 12 needs live browser verification; Tasks 13–14 need harness/release verification. The deferred Effect rc.112 early-Retry lost-wakeup regression remains a release blocker. No release, merge, push, or production cutover has occurred.
